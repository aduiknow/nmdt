...
+ Vấn đề khi làm việc cá nhân: bạn đang làm việc với một số tập tin và thư mục nào đó (gọi đơn giản là files). Hàng ngày, bạn tiến hành nhiều thay đổi trên các files này. Vào một ngày xấu trời nào đó, bạn cần quay lại thời điểm của những files này cách đây 2 ngày thì bạn phải làm sao?\vskip 0.4cm

+ Vấn đề khi làm việc nhóm: bạn và vài người bạn của bạn đang làm việc trên một số files (project), làm sao để nhiều người làm việc hiệu quả trên các files này (không có những trường hợp ghi đè nội dung của nhau, xóa nhầm files, quản lý được ai thao tác trên các files nào... )\vskip 0.4cm

Từ hai vấn đề trên các thành viên trong nhóm đã tìm hiểu trên nhiều nguồn tài liệu và biết đến công cụ quản lí mã nguồn GIT có thể giải quyết được các vấn đề trên và hơn thế nữa. Sau khi có được chủ đề bài tập lớn, nhóm đã cùng nhau tìm hiểu về lịch sử ra đời của GIT, cách Download và học cách sử dụng GIT một cách hiệu quả. Sau thời gian học tập và tìm kiếm tài liệu trên mạng, thông qua quá trình thực hành nhóm đã có những kiến thức cơ bản nhất. Và dưới đây chính là những kiến thức mà nhóm đã tìm hiểu được về GIT.
...
\hspace{0.6cm} •	Quản lý phiên bản (VCS) là một hệ thống ghi lại những thay đổi trên một tệp hay tập hợp các tệp theo thời gian để mà chúng ta có thể khôi phục lại các phiên bản trước.
\vskip 0.4cm
...
- Git có một công cụ có sẵn gọi là git config, cho phép ta xem và điều chỉnh các biến cấu hình để điều khiển được mọi khía cạnh của Git \vskip 0.4cm
...
+ Để đặt tên người dùng, ta dùng lệnh:{\it  git config --global user.name “Your name”} \vskip 0.4cm
+ Để đặt email: {\it git config --global user.email john@example.com}\vskip 0.4cm
%\begin{figure}
	%\centering
...
{\it git config --global core.editor “ ’[đường dẫn đến công cụ trong máy bạn]’ -multilnst -nosession”}\vskip 0.4cm
...
- Khi muốn kiểm tra các cài đặt, ta dùng lệnh git config --list. Kết quả như hình bên dưới: 
\vskip 0.4cm
...
- Repository, hay còn gọi là Repo, dịch ra tiếng Việt nghĩa là kho. Repo là nơi chứa tất cả mã nguồn cho một dự án được quản lý bởi Git.\vskip 0.4cm
- Nếu bạn muốn bắt đầu quản lý một dự án có sẵn bằng Git, bạn cần di chuyển đến thư mục đó sử dụng bash và gõ: git init\vskip 0.4cm
...
- Lệnh git status dùng để kiểm tra trạng thái của thư mục đang làm việc hiện tại.\vskip 0.4cm
...
- Câu lệnh{\it git diff - -staged} đã được tổ chức (staged) và chuẩn bị được commit.\vskip 0.4cm
{\bf e. Commit các thay đổi}\vskip 0.4cm
- Sau khi đã tổ chức các tập tin theo ý muốn, chúng ta có thể commit chúng\vskip 0.4cm
- Lưu ý rằng những thứ chưa được tổ chức (unstaged) – những file được tạo ra hoặc chỉnh sửa sau khi bạn chạy lệnh git add sẽ không được commit, do những file vẫn đang ở trạng thái modified.\vskip 0.4cm
- Cách đơn giản để commit đó là dùng lệnh:{\it git commit –m “nội dung của lần commit này”}\vskip 0.4cm
+ Sau khi đã đưa các tệp lên trạng thái staged ta phải sử dụng lệnh:{\it git commit -m “initial project version”} để commit các tệp đang ở trạng thái staged\vskip 0.4cm
...
- Thêm lựa chọn -a vào câu lệnh git commit sẽ làm Git tự động tổ chức (stage) tất cả những tệp đã được theo dõi trước khi thực hiện commit, cho phép ta bỏ qua bước git add\vskip 0.4cm
- Có thế add hết tất cả các tệp untracked vào staged bằng lệnh:{\it git add - -all}\vskip 0.4cm

	\includegraphics[width=0.8\linewidth]{screenshot013}

	\label{fig:screenshot013}
\vskip 0.4cm\vskip 0.4cm
- Ở hình trên, có 3 tệp untracked, sử dụng{\it git add --all} để đưa cả 3 tệp vào stage area cùng lúc\vskip 0.4cm
...
- {\it git rm - -cached} : tập tin không còn bị kiếm soát bởi Git nhưng mà vẫn còn trên đĩa cứng\vskip 0.4cm
...
\vskip 0.4cm\vskip 0.4cm
+ Ở hình trên chỉ hiển thị 2 hành động gần nhất nhưng có thêm một vài thông số. Ví dụ: bạn có thể sử dụng lệnh{\it diff --git a/5.txt b/5.txt} để so sánh sự thay đổi của tệp 5.txt (a b ở đây chỉ phiên bản của tệp)\vskip 0.4cm

- Nếu ta muốn xem một bản thống kê tóm tắt cho mỗi commit, ta dùng lựa chọn –stat. Lệnh {\it git log –stat} sẽ in ra phía dưới mỗi commit danh sách các tập tin đã chỉnh sửa, bao nhiêu tập tin được sửa, và bao nhiêu dòng trong các tập tin đó được thêm vào hay xoá đi.\vskip 0.4cm
...
- Một lựa chọn hữu ích khác là –pretty. Lựa chọn này thay đổi phần hiển thị theo các cách khác nhau.\vskip 0.4cm
+ Lựa chọn oneline in ra mỗi commit trên một dòng, thuận tiện cho việc xem nhiều commit: {\it git log –pretty=oneline}\vskip 0.4cm

	\includegraphics[width=0.8\linewidth]{screenshot020}
	
	\label{fig:screenshot020}
\vskip 0.4cm\vskip 0.4cm
+ Ta đã sử dụng {\it option - -pretty=oneline} để hiển thị mỗi commit chỉ là một dòng gồm commit ID và tin nhắn của mỗi lần commit\vskip 0.4cm
+ Lựa chọn thú vị nhất là {\it –pretty=format}, cho phép ta chỉ định định dạng riêng của phần hiển thị.
\vskip 0.4cm
	\includegraphics[width=0.8\linewidth]{screenshot021}

	\label{fig:screenshot021}
\vskip 0.4cm\vskip 0.4cm
+ Ví dụ: {\it git log --pretty=format:”\%h - \%an, \%ar : \%s”}. Kết quả thu được\vskip 0.4cm
...
- Các lựa chọn giới hạn thời gian thông tin đầu ra như –since và –until khá hữu ích\vskip 0.4cm

	\includegraphics[width=0.8\linewidth]{screenshot023}

	\label{fig:screenshot023}
\vskip 0.4cm\vskip 0.4cm
+ Ví dụ: lệnh để hiển thị các commit từ 2 tuần trước trở lại do Nguyen Quy tạo ra:{\it git log --pretty="\%h - \%s" --author="Nguyen Quy" --since=2.week }. Kết quả sẽ được:\vskip 0.4cm
